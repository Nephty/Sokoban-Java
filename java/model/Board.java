...
public class Board {
    ...
    */
    ...
    public boolean isWin(){
        return ((currBoxOnObj == goals.size()|| (currBoxOnObj == boxes.size())));
    }
    ...
    public void move(Direction direction){
        ...
        }else if (nextObj instanceof Box){
            //if nothing is above the box
            if (moveBox(nextObj, blockList[nextY2][nextX2], nextY, nextX, nextX2, nextY2, direction)){
                move(direction);
            }
            //nothing is above the player
        }else if (nextObj == null){
            //a player is on an goal so he change to Player object
            if (player1 instanceof PlayerOnObj){
