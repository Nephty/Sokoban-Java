...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        EventHandler key = new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent event) {
                ...
                if (direction != Direction.NULL) {
                    ...
                    if (direction == Direction.RESTART){
                        restartMap(totalMovesText, totalPushesText, objectivesText, gamePane);

                    }else if (moveResult.isA()) {
                        totalMoves++;
                        totalMovesText.setText(String.valueOf(totalMoves));
                        if (moveResult.isB()) {
                            totalPushes++;
        ...
        levelPane.setPrefWidth(1570*WR);
        levelPane.setMaxWidth(1570*WR);
        levelPane.setMinWidth(1570*WR);
        levelPane.setPrefHeight(windowHeight);
        levelPane.setMaxHeight(windowHeight);
        levelPane.setMinHeight(windowHeight);
        levelPane.setLayoutX(WR);
        levelPane.setLayoutY(0);
        Rectangle midRR = new Rectangle(0, 0, 1570*WR, windowHeight*HR);
        midRR.setFill(Color.BLACK);

        Pane rightViewer = new Pane();
        rightViewer.setPrefWidth(350*WR);
        rightViewer.setMaxWidth(350*WR);
        rightViewer.setMinWidth(350*WR);
        rightViewer.setPrefHeight(windowHeight);
        rightViewer.setMaxHeight(windowHeight);
        rightViewer.setMinHeight(windowHeight);
        rightViewer.setLayoutX(1570*WR);
        rightViewer.setLayoutY(0);
        CustomImage rightviewerImage = new CustomImage(0, 0, WR, HR, "right side menu.png");
        rightViewer.getChildren().add(rightviewerImage);

        Pane viewPane = new Pane();
        viewPane.setPrefWidth(350*WR);
        viewPane.setMaxWidth(350*WR);
        viewPane.setMinWidth(350*WR);
        viewPane.setPrefHeight(350*HR);
        viewPane.setMaxHeight(350*HR);
        viewPane.setMinHeight(350*HR);
        viewPane.setLayoutX(0);
        viewPane.setLayoutY(350*HR);


        CustomButton level01 = new CustomButton( 100,125,WR, HR, "achievement1.png");
        CustomButton level02 = new CustomButton( 100,225,WR,HR, "achievement2.png");
        CustomButton level03 = new CustomButton( 100,325,WR,HR,"achievement3.png");
        CustomButton level04 = new CustomButton( 100, 425, WR, HR, "achievement11.png");
        CustomButton level05 = new CustomButton( 100, 525,WR,HR, "achievement22.png");
        CustomButton level06 = new CustomButton( 100, 625,WR,HR, "achivement1 done.png");
        level01.setOnMouseClicked(e -> {
            try {
                chooseMap("level01.xsb", gamePane);
                window.setScene(gameUI);
                window.setFullScreen(fullscreen);
            }catch(IOException exception){
            }
        });
        level01.setOnMouseEntered(e -> {
            try{
                setMapView("maps\\lvl01.png", viewPane);
            }catch (IOException exception){
                exception.printStackTrace();
            }
        });
        level01.setOnMouseExited(e -> {
            viewPane.getChildren().removeAll(viewPane.getChildren());
        });

        level02.setOnMouseClicked(e -> {
            try{
                chooseMap("level02.xsb", gamePane);
                window.setScene(gameUI);
                window.setFullScreen(fullscreen);
            }catch (IOException exception){
            }
        });
        level02.setOnMouseEntered(e -> {
            try{
                setMapView("maps\\lvl02.png", viewPane);
            }catch (IOException exception){
                exception.printStackTrace();
            }
        });
        level02.setOnMouseExited(e -> {
            viewPane.getChildren().removeAll(viewPane.getChildren());
        });

        level03.setOnMouseClicked(e -> {
            try{
                chooseMap("level03.xsb", gamePane);
                window.setScene(gameUI);
                window.setFullScreen(fullscreen);
            }catch(IOException exception){
            }
        ...
        level03.setOnMouseEntered(e -> {
            try{
                setMapView("maps\\lvl03.png", viewPane);
            }catch (IOException exception){
                exception.printStackTrace();
            }
        });
        level03.setOnMouseExited(e -> {
            viewPane.getChildren().removeAll(viewPane.getChildren());
        });

        level04.setOnMouseClicked(e ->{
            try{
                chooseMap("level04.xsb", gamePane);
                window.setScene(gameUI);
                window.setFullScreen(fullscreen);
            }catch (IOException exception){
            }
        });
        level04.setOnMouseEntered(e -> {
            try{
                setMapView("maps\\lvl04.png", viewPane);
            }catch (IOException exception){
                exception.printStackTrace();
            }
        });
        level04.setOnMouseExited(e -> {
            viewPane.getChildren().removeAll(viewPane.getChildren());
        });

        level05.setOnMouseClicked(e ->{
            try{
                chooseMap("level05.xsb", gamePane);
                window.setScene(gameUI);
                window.setFullScreen(fullscreen);
            }catch (IOException exception){
            }
        });
        level05.setOnMouseEntered(e -> {
            try{
                setMapView("maps\\lvl05.png", viewPane);
            }catch (IOException exception){
                exception.printStackTrace();
            }
        });
        level05.setOnMouseExited(e -> {
            viewPane.getChildren().removeAll(viewPane.getChildren());
        });

        level06.setOnMouseClicked(e ->{
            try{
                chooseMap("level06.xsb", gamePane);
                window.setScene(gameUI);
                window.setFullScreen(fullscreen);
            }catch (IOException exception){
            }
        });
        level06.setOnMouseEntered(e -> {
            try{
                setMapView("maps\\lvl06.png", viewPane);
            }catch (IOException exception){
                exception.printStackTrace();
            }
        });
        level06.setOnMouseExited(e -> {
            viewPane.getChildren().removeAll(viewPane.getChildren());
        });

        rightViewer.getChildren().add(viewPane);

        levelPane.getChildren().addAll(midRR,level01, level02,
                level03,level04,level05,level06);

        Pane finalLevelPane = new Pane();
        finalLevelPane.setPrefWidth(windowWidth);
        finalLevelPane.setPrefHeight(windowHeight);
        finalLevelPane.getChildren().addAll(levelPane,rightViewer);

        levelMenu = new Scene(finalLevelPane, windowWidth, windowHeight);
    ...
    public void prepareMapSize(Board board) {
    ...
    private Board chooseMap(String file, Pane gamePane) throws IOException{
        ArrayList<String> level = Fichier.loadFile(file, "");
        firstBoard = new Board(level);
        blockList = firstBoard.getBlockList();
        prepareMapSize(firstBoard);
        setMap(firstBoard, gamePane);
    ...
    private void setMapView(String image, Pane viewPane) throws IOException{
        viewPane.getChildren().removeAll(viewPane.getChildren());
        CustomImage viewImg = new CustomImage(0,0,WR,HR,image);
        viewImg.setLayoutX(viewImg.getX_());
        viewImg.setLayoutY(viewImg.getY_());
        viewImg.setWidth(viewImg.getWidth_());
        viewImg.setHeight(viewImg.getHeight_());
        viewPane.getChildren().add(viewImg);
    }

    private void restartMap(Text totalMovesText, Text totalPushesText, Text objectivesText, Pane gamePane){
        firstBoard.restart();
        playerFacing = Direction.DOWN;
        totalMoves = 0;
        totalPushes = 0;
    ...
    public void setMap(Board board, Pane gamePane) throws FileNotFoundException {
        ...
        }else if (board.isOnPressurePlate()){
            window.setScene(mainMenu);
        }
    }
